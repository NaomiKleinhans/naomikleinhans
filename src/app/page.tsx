...
const Home = () => {
    ...
    const handleScrollButtonClick = (direction: string) => {
        ...
        const newScrollLeft = direction === 'right' ? container.scrollLeft + container.clientWidth : container.scrollLeft - container.clientWidth;
    ...
    return (
        <div key='1' className='flex flex-col bg-[#181818] overflow-y-hidden'>
            <MainNav />
            <MobileMainNav />
            <main>
                {canScrollLeft ? (
                ...
                {canScrollRight ? (
                    <div className='w-10 h-10 absolute top-1/2 right-0 z-50 cursor-pointer animate-pulse'>
                        <ChevronRightIcon onClick={() => handleScrollButtonClick('right')} />
                    </div>
                ) : (
                    <></>
                )}

                <div className='carousel-container overflow-x-scroll h-content flex no-scrolly' ref={containerRef}>
                    <section className='min-w-full lg:space-y-10 md:space-y-10 sm:space-y-10 bg-[#181818]' id='home'>
                        ...
                        <div className='sm:hidden'>
                            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '30vh', width: 'auto' }}>
                                <div style={{ border: '6px solid #5A75CE', borderRadius: '50%', overflow: 'hidden' }}>
                                    <Image src='/profile-pic.png' alt='Profile Picture' width='250' height='250' />
                                </div>
                            </div>
                        </div>
                        <div className='lg:hidden md:hidden mt-16'>
                            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '30vh', width: 'auto' }}>
                                <div style={{ border: '6px solid #5A75CE', borderRadius: '50%', overflow: 'hidden' }}>
                                    <Image src='/profile-pic.png' alt='Profile Picture' width='200' height='200' />
                                </div>
                            </div>
                        </div>
                        <div className='text-center text-textColor lg:text-lg md:text-md sm:text-xs'>
                            Explore my{' '}
                            <a href='#featured-projects' className='button border-themeColorMain border-2 text-themeColorMain px-2 py-1 font-semibold rounded-md mx-2'>
                                Projects
                            </a>{' '}
                        </div>
                    </section>
                    <section className='min-w-full flex justify-center bg-[#181818]' id='featured-projects'>
                        <Project />
                    </section>
                    <section className='min-w-full flex justify-center bg-[#181818]' id='about'>
                        <About />
                    </section>
                    <section className='min-w-full flex justify-center bg-[#181818]' id='contact'>
                        <Contact />
                    </section>
                </div>
            </main>
