// Top Navbar Icons

export * from './NavBellIcon'
export * from './NavEventsIcon'
export * from './NavOptionsIcon'

// Main Menu Icons

export * from './DashboardIcon'
export * from './DashboardActiveIcon'
export * from './ProfileIcon'
export * from './ProfileActiveIcon'
export * from './InMailIcon'
export * from './InboxActiveIcon'
export * from './BlogIcon'
export * from './BlogsActiveIcon'
export * from './LessonIcon'
export * from './LessonActiveIcon'
export * from './MainNotesIcon'
export * from './KnowledgeBaseIcon'
export * from './KnowledgeBaseActiveIcon'
export * from './ApplicationsIcon'
export * from './Hamburger'
export * from './BursaryIcon'
export * from './BursariesActiveIcon'
export * from './NotesIcon'
export * from './NotesActiveIcon'
export * from './AvatarIcon'
export * from './MailPlaneIcon'

// Support Icons

export * from './LogTicketIcon'
export * from './LogTicketActiveIcon'
export * from './TicketIcon'
export * from './TicketsActiveIcon'
export * from './NotificationIcon'

// User stats

export * from './StatsCompletedIcon'
export * from './StatsHoursIcon'
export * from './StatsInprogressIcon'
export * from './AverageQuizScoreIcon'
export * from './CloseIcon'

// Subject Icons

export * from './AccSubjIcon'
export * from './BusStudSubjIcon'
export * from './EconSubjIcon'
export * from './EngFALSubjIcon'
export * from './EngHLSubjIcon'
export * from './GeoSubjIcon'
export * from './LifeSciSubjIcon'
export * from './MathSubjIcon'
export * from './MathLitSubjIcon'
export * from './PhySciSubjIcon'
export * from './TourismSubjIcon'

// In mail icons

export * from './ImNewMailIcon'
export * from './ImInboxIcon'
export * from './ImSentIcon'
export * from './ImDraftIcon'
export * from './ImTrashIcon'
export * from './ImStarredIcon'
export * from './ImImportantIcon'
export * from './TopTrashIcon'
export * from './TopRecycleTrashIcon'
export * from './ImDownArrowIcon'
export * from './ImOptionsIcon'
export * from './StarredIcon'
export * from './DefaultStarredIcon'
export * from './ImportantBlueIcon'
export * from './ImportantDefaultIcon'
export * from './ImModalCloseIcon'
export * from './ImAttachmentIcon'
export * from './PullLeftIcon'
export * from './PullRightIcon'
export * from './LessonBullet'
export * from './AddressBook'
export * from './LikesIcon'
export * from './CommentsIcon'
export * from './ImMarkReadIcon'
export * from './BurgerBarIcon'
export * from './BurgerBarCloseIcon'
export * from './MoreIcon'

//Account info icons

export * from './BioIcon'
export * from './ContactIcon'
export * from './SchoolIcon'
export * from './DeviceIcon'
export * from './NoKIcon'
export * from './ChevronDownIcon'
export * from './ChevronRightIcon'
export * from './ChevronLeftIcon'
export * from './ShareIcon'
export * from './HeartIcon'

//lms

export * from './ChevUp'
export * from './ChevDown'
export * from './LmsHelpfulIcon'

// Applications Icons

export * from './BriefcaseIcon'
export * from './BuildingIcon'
export * from './LightbulbIcon'
export * from './ForwardIcon'
export * from './SendIcon'
export * from './ShowsIcon'
export * from './ShowsActiveIcon'
export * from './DigilibIcon'

// Social Icons

export * from './FacebookActiveIcon'
export * from './FacebookIcon'
export * from './ForumActiveIcon'
export * from './ForumIcon'
export * from './InstagramIcon'
export * from './InstagramActiveIcon'
export * from './PlaystoreIcon'
export * from './TikTokIcon'
export * from './TikTokActiveIcon'
export * from './WhatsAppIcon'
export * from './WhatsAppActiveIcon'
export * from './WhatsAppGroupIcon'
export * from './WhatsAppGroupActiveIcon'
